...
func (ctx *Context) XSRFToken(specifiedExpiration ...int) string {
	if ctx._xsrfToken == "" {
		token, ok := ctx.SecureCookieParam(ctx.engine.config.XSRF.Key, "_xsrf")
		if !ok {
			...
			if len(specifiedExpiration) > 0 && specifiedExpiration[0] > 0 {
				...
				ctx.xsrfExpire = ctx.engine.config.XSRF.ExpireSecond
			}
		}
...
func (ctx *Context) checkXSRFCookie() bool {
	...
	if token == "" {
		token, _ = ctx.SecureCookieParam(ctx.engine.config.XSRF.Key, "_xsrf")
	}
	if token == "" {
...
func (ctx *Context) startSession() (session.Store, error) {
	...
	var err error
	ctx.curSession, err = ctx.engine.sessionManager.SessionStart(ctx.W, ctx.R)
...
func (ctx *Context) getSessionStore() (session.Store, error) {
	...
	var err error
	ctx.curSession, err = ctx.engine.sessionManager.GetSessionStore(ctx.W, ctx.R)
	return ctx.curSession, err
}
...
func (ctx *Context) SessionRegenerateID() {
	...
	ctx.curSession = ctx.engine.sessionManager.SessionRegenerateID(ctx.W, ctx.R)
}
...
func (ctx *Context) DestroySession() {
	...
	ctx.curSession = nil
	ctx.engine.sessionManager.SessionDestroy(ctx.W, ctx.R)
}
...
func (ctx *Context) doFilter() bool {
	if count := len(ctx.engine.filter); count > 0 {
		ctx.handlerChain = ctx.engine.filter
...
func (ctx *Context) beforeWriteHeader() {
	if ctx._xsrfTokenReset {
		ctx.SetSecureCookie(ctx.engine.config.XSRF.Key, "_xsrf", ctx._xsrfToken, ctx.xsrfExpire)
	}
	if ctx.enableSession {
		if ctx.curSession != nil {
			ctx.curSession.SessionRelease(ctx.W)
...
func (ctx *Context) recordBody() []byte {
	if !ctx.engine.config.PrintBody {
		return nil
	}
	var b []byte
...
func (e *Engine) getContext(w http.ResponseWriter, r *http.Request) *Context {
	ctx := e.contextPool.Get().(*Context)
	ctx.R = r
	ctx.W.reset(w)
	ctx.data = make(map[interface{}]interface{})
	if e.config.PrintBody && !ctx.IsUpload() {
